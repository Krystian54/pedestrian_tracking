...
# Algorytm węgierski: pozycja, prędkość, hog, wielkość ramki
...
def bbox_size(bbox):
    x1, y1, x2, y2 = bbox
    return np.array([np.abs(x2 - x1), np.abs(y2 - y1)])
...
def hungarian_algorithm(input_path, output_path, hog_ratio_gui, dst_ratio_gui, speed_ratio_gui, size_ratio_gui, cost_threshold, max_age):
    
    cap = cv2.VideoCapture(input_path)

    # parametry
    hog_ratio = 0.1             *   1    * hog_ratio_gui    #2.0
    dst_ratio = 1/(1920/3)      *   0.5  * dst_ratio_gui    #0.5
    speed_ratio = 1/(1920/3)    *   0.5  * speed_ratio_gui  #0.5
    size_ratio = 10             *   1    * size_ratio_gui   #1.0
    ...
    while cap.isOpened():
        ...
        # obliczenie hog dla masek po erozji
