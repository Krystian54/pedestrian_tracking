...
def compute_hog_masked(frame, bbox, mask):
    ...
    return hog_descriptor.compute(roi_resized)

# obliczenie kosztu
def combined_cost(hog1, hog2, bbox1, bbox2, speed1, hog_ratio, dst_ratio, speed_ratio):
...
def hungarian_algorithm(input_path, output_path, hog_ratio, dst_ratio, speed_ratio, cost_threshold, max_age):

    cap = cv2.VideoCapture(input_path)

    # parametry
    hog_ratio = 0.1 *           1    * hog_ratio
    dst_ratio = 1/(1920/3) *    0.5  * dst_ratio
    speed_ratio = 1/(1920/3) *  0.5  * speed_ratio
